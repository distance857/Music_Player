                    ...
                    <ul class="queue-wrap">
                        <el-scrollbar style="height:100%">
                            <li class="queue-song" v-for="item in musicQueue" :key="item.id" @dblclick="play(item)"
                                ...
                                <span
                                    :class="{ 'playingIcon iconfont icon-zanting': isMusicPaused && item.id == globalMusicInfo.id }"></span>
                                <span class="queue-song-name">{{ item.songName }}</span>

                                <div class="queue-song-singer">
                                    <!-- <span v-for="(artistname, i) in item.artistname" :key="i"
                                        @click.stop="toArtist(1)">{{ artistname }} </span> -->
                                    <span class="queue-song-name">{{ item.artistname }}</span>
                                </div>
...
<script>
import { getSongsByIdAPI } from '@/utils/api'
...
export default {
    ...
    methods: {
        ...
        play(item) {
            console.log("11111")
            getSongsByIdAPI(item.id).then(res => {
                if (1) {
                    let songUrl = "http://music.163.com/song/media/outer/url?id=" + item.id + ".mp3"
                    let musicInfo = {
                        id: item.id,
                        imgUrl: item.imgUrl,
