        ...
        <div class="music-box">
            ...
            <div class="music-info" v-show="globalMusicInfo.songName">
                ...
                <div class="music-singer">
                    <!-- <span v-for="(singer, i) in globalMusicInfo.artistInfo" :key="i + 99">{{ singer.artistname }} </span> -->
                    <span>{{ globalMusicInfo.artistname }} </span>
                </div>
            </div>
            <span class="music-name" v-show="!globalMusicInfo.songName" style="line-height:50px">还没有播放音乐哦</span>
        </div>
        <AudioChen :musicUrl="globalMusicUrl" @timeupdate="updateTime" @play="playStatus" @pause="pauseStatus"
            @next="next" @prev="prev" @end="next"></AudioChen>
        <div @click="showQueue($event)" ref="queue" title="播放队列" :class="{
                'delete-queue-style bofangliebiao'
...
export default {
    ...
    watch: {
        ...
        globalMusicInfo() {
            ...
            console.log(this.globalMusicInfo);
